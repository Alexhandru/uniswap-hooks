...
contract VolumeBasedFeeHookTest is Test, Deployers {
    ...
    function test_fuzz_swapParams(
        ...
        uint24 expectedFee = swapVolume.exposed_calculateFee(
            SwapParams({
                ...
                sqrtPriceLimitX96: zeroForOneFuzz == 1 ? MIN_PRICE_LIMIT : MAX_PRICE_LIMIT
            })
        );
    ...
    function _setupFuzzedParams(
        ...
        minAmount0 = bound(_minAmount0, 1, 10e18 - 1);
        maxAmount0 = bound(_maxAmount0, minAmount0 + 1, 10e18);
        minAmount1 = bound(_minAmount1, 1, 10e18 - 1);
        maxAmount1 = bound(_maxAmount1, minAmount1 + 1, 10e18);
    ...
    function _setupAmountSpecified(int256 _amountSpecified) private {
        vm.assume(amountSpecifiedFuzz > 0);

        bool exactInput = amountSpecifiedFuzz < 0 ? true : false; // Check if it's an exact input swap
        int256 minAmount = zeroForOneFuzz == 1 ? 
            (exactInput ? int256(minAmount0) : int256(minAmount1)) : 
            (exactInput ? int256(minAmount1) : int256(minAmount0));
        int256 maxAmount = zeroForOneFuzz == 1 ? 
            (exactInput ? int256(maxAmount0) : int256(maxAmount1)) : 
            (exactInput ? int256(maxAmount1) : int256(maxAmount0));

        if (amountSpecifiedFuzz % 3 == 0) {
            amountSpecifiedFuzz = bound(
                amountSpecifiedFuzz,
                int256(1),
                minAmount == 1 ? int256(1) : minAmount - 1 
            );
        } else if (amountSpecifiedFuzz % 3 == 1) {
            amountSpecifiedFuzz = bound(
                amountSpecifiedFuzz,
                minAmount,
                maxAmount
            );
        } else if (amountSpecifiedFuzz % 3 == 2) {
            amountSpecifiedFuzz = bound(
                amountSpecifiedFuzz,
                maxAmount + 1, 
                int256(1e40)
    ...
    function _validateParams(VolumeBasedFeeHook.SwapVolumeParams memory paramsOut) view private {
        ...
        assertEq(paramsOut.feeAtMinAmount1, feeAtMinAmount1, "feeAtMinAmount1 mismatch");
        assertEq(paramsOut.feeAtMaxAmount1, feeAtMaxAmount1, "feeAtMaxAmount1 mismatch");
...
contract VolumeBasedFeeHookHarness is VolumeBasedFeeHook {
    ...
    constructor(IPoolManager _poolManager, SwapVolumeParams memory params) VolumeBasedFeeHook(_poolManager, params) {}
